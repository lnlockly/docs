...
<CardGroup cols={2}>
  ...
  <Card title="Billing surface" icon="receipt">
    Monetization model and per-call price. See [Monetization Models](/marketplace/monetization-models).
  </Card>
</CardGroup>
...
The buckets are summed and debited from the caller's FLOW balance. They are then split across creator / holders / platform / reserve according to the rules in [Earnings Split](/marketplace/earnings-split).
...
See [Self-Host](/self-host/architecture) for the manifest spec.
