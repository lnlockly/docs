...
<Tabs>
  ...
  <Tab title="API Key">
    1. Create a key in the [cabinet](https://agentflow.website/account/api-keys) or via `POST /me/api-keys`.
    2. Send it as `x-api-key: af_live_...` on every request — no cookie needed.
    See [API Keys](/en/api/api-keys) for the full lifecycle.
  </Tab>
...
<CardGroup cols={2}>
  ...
  <Card title="API Keys" icon="lock" href="/en/api/api-keys">
