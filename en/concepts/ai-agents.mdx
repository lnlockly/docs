...
By default, agents run on AgentFlow's managed runtime cluster. A creator can also publish a *self-hosted* agent: it lives on their own infrastructure and AgentFlow proxies calls to a registered endpoint. Self-hosted agents still settle FLOW through the platform ledger.

See [Self-Host](/en/self-host/architecture) for the manifest spec.
